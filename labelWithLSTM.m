function [] = labelWithLSTM(app)
%Label the entire loaded dataset using a pre-trained LSTM model, Oliver
%Pambos, 19/01/2024.
%oliver.pambos@physics.ox.ac.uk
%
%
%MATLAB FUNCTION: labelWithLSTM
...
%
%
%This function temporarily replicates the function labelWithGRU() for
...
%cropTrajectories()
    ...
    for ii = 1:size(app.movie_data.results.LSTMLabelled.LabelledMols, 1)
        ...
        if size(curr_track, 1) <app.movie_data.models.LSTM.max_len
            padding = zeros(app.movie_data.models.LSTM.max_len - size(curr_track, 1), size(curr_track, 2));
            curr_track = [curr_track; padding];
        elseif size(curr_track, 1) > app.movie_data.models.LSTM.max_len
            curr_track = curr_track(1:app.movie_data.models.LSTM.max_len, :);
        end
    ...
    t = toc;
    app.textout.Value = "Completed classification and segmentation of entire dataset using LSTM model in " + num2str(t) + " seconds";
    %close(f);
end
