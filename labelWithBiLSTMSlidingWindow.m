function [] = labelWithBiLSTMSlidingWindow(app)
...
%
%
%This function is a temporary copy of labelWithLSTMSlidingWindow and will
%be removed during a future refactoring.
%
...
%-----------------------------------------
%cropTrajectories()
%labelSingleTrackSlidingBiLSTM()   - local to this .m file
    ...
    f = waitbar(0,"Labelling the entire loaded dataset using Long Short-Term Memory (BiLSTM) model...");
    %update_freq = 5;
    ...
    for ii = 1:size(app.movie_data.results.BiLSTMLabelled.LabelledMols, 1)
        ...
        label_sequence = labelSingleTrackSlidingBiLSTM(curr_track, app.movie_data.models.BiLSTM.max_len, numel(app.movie_data.models.BiLSTM.class_names), app.movie_data.models.BiLSTM.model);
        ...
        %if mod(ii, update_freq) == 0
            waitbar(ii/size(app.movie_data.results.BiLSTMLabelled.LabelledMols, 1), f, "Classificaiton complete for " + num2str(ii) + "/" + num2str(size(app.movie_data.results.BiLSTMLabelled.LabelledMols, 1)) + " trajectories");
        %end
    end
...
function [final_predictions] = labelSingleTrackSlidingBiLSTM(curr_track, window_size, N_classes, model)
%Uses a sliding window to label a single trajectory using a BiLSTM model,
%Oliver Pambos, 11/02/2024.
%oliver.pambos@physics.ox.ac.uk
%
%
%MATLAB FUNCTION: labelSingleTrackSlidingBiLSTM
%AUTHOR: OLIVER JAMES PAMBOS, DEPARTMENT OF PHYSICS, UNIVERSITY OF OXFORD, UK
%CONTACT: oliver.pambos@physics.ox.ac.uk
%
%LEGAL DISCLAIMER
...
%model          (mdl)   BiLSTM model used for classification
%
%
%Output
%------
