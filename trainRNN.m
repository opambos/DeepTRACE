...
%genChangepointMasks()      - local to this .m file
    ...
    options = getTrainingOptions(val_data_tp, val_labels_tp, true, app.movie_data.state.training.max_epochs,...
                                                                   ...
                                                                   app.movie_data.state.training.LR_drop_period,...
                                                                   app.movie_data.state.training.LR_drop_fraction);
...
%Inputs
%------
%val_data           (cell)  validation data as a cell array in which each
%                               cell contains a separate example in the
%                               form of a matrix of dimension TxF, where T
%                               is the number of timepoints, and F is the
%                               number of features
%val_labels         (cell)  cell array of validation labels containing the
%                               known classes for each datapoint in
%                               val_data, each cell contains a matrix of
%                               dimension Tx1, where T is the number of
%                               timepoints
%show_plot          (bool)  determines whether to displain the training
%                               process graphically to user
%max_epochs         (int)   maximum number of epochs
%batch_size         (int)   number of examples to include in each batch
%learn_rate         (float) initial learning rate at the start of training
%val_interval       (int)   number of iterations between validation tests
%val_patience       (int)   maximum number of consecutive evaluations of
%                               the validations dataset without improvement
%                               before early stopping
%LR_drop_period     (int)   number of epochs between step down in learning
%                               rate
%LR_drop_fraction   (float) fraction (range 0 - 1) multiplied by the
%                               learning rate at each event in which the
...
%None
    ...
    options = trainingOptions('adam', ...
        ...
        'LearnRateDropFactor', LR_drop_fraction, ...
